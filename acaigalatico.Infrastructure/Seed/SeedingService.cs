...
namespace acaigalatico.Infrastructure
{
    public class SeedingService
    {
        ...
        private async Task EnsureMockOrdersAsync()
        {
            if (_context.Vendas.Any())
                return;

            var clientesBase = new List<Cliente>
            {
                new Cliente { Nome = "João Silva", Telefone = "(11) 98888-7777", SaldoDevedor = 0m },
                new Cliente { Nome = "Maria Oliveira", Telefone = "(11) 97777-6666", SaldoDevedor = 0m },
                new Cliente { Nome = "Pedro Santos", Telefone = "(11) 96666-5555", SaldoDevedor = 0m }
            };

            foreach (var clienteBase in clientesBase)
            {
                if (!_context.Clientes.Any(c => c.Telefone == clienteBase.Telefone))
                {
                    _context.Clientes.Add(clienteBase);
                }
            }
            ...
            var pedidosFicticios = new List<Venda>
            {
                new Venda
                {
                    ValorTotal = 25.00m,
                    ...
                    BairroEntrega = "Jardins",
                    DataVenda = DateTime.Now.AddHours(-2),
                    ClienteId = clientes[0].Id,
                    FormaPagamento = TipoPagamento.Pix,
                    Observacao = "Pedido fictício para validar status de novo pedido"
                },
                new Venda
                {
                    ValorTotal = 18.50m,
                    Status = StatusVenda.Preparando,
                    EnderecoEntrega = "Av. Paulista, 1000",
                    BairroEntrega = "Bela Vista",
                    DataVenda = DateTime.Now.AddHours(-1),
                    ClienteId = clientes[1].Id,
                    FormaPagamento = TipoPagamento.Dinheiro,
                    Observacao = "Pedido fictício para validar status em andamento"
                },
                new Venda
                {
                    ValorTotal = 32.00m,
                    ...
                    FormaPagamento = TipoPagamento.Cartao,
                    Observacao = "Pedido fictício para validar status entregue"
                }
            };

            _context.Vendas.AddRange(pedidosFicticios);
            await _context.SaveChangesAsync();
        }
    }
}
